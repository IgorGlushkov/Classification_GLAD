...
print "gdal_rasterize -a CLASSFIN -of EHdr -ot Byte -te $ulx $lry $lrx $uly -ts $tileside $tileside -l target $proj\\target.shp $proj\\$tile.bil  \n";
system("gdal_rasterize -a CLASSFIN -of EHdr -ot Byte -te $ulx $lry $lrx $uly -ts $tileside $tileside -l target $proj\\target.shp $proj\\$tile.bil");
}
#die;
}
