...
$memsize=9000000000; # $settings{memsize};   Stack memory allocation
...
open (DAT, "tilelist3.txt"); @tilelist=<DAT>; close (DAT); foreach (@tilelist) {chomp;}
#&subset_tile_list;
###########################################################################
# Setup #
$|=1; use threads;
...
print "gdal_rasterize -a val -of EHdr -ot Byte -te $ulx $lry $lrx $uly -ts $tileside $tileside -l reg $proj\\reg.shp $proj\\$tile.bil  \n";
system("gdal_rasterize -a val -of EHdr -ot Byte -te $ulx $lry $lrx $uly -ts $tileside $tileside -l reg $proj\\reg.shp $proj\\$tile.bil");
...
//source data
uint8_t training[xsize][ysize]; memset(training, 0, sizeof(training[0][0]) * xsize * ysize);
uint8_t mask_t[xsize][ysize];
unsigned short pf[xsize][ysize];
";
foreach (@mainlist){print OUT "uint8_t $_\[xsize][ysize];\n";}
foreach (@diff){($out,$in1,$in2)=split('\,',$_); print OUT "unsigned short $out\[xsize][ysize];\n";}
foreach (@list16s) {print OUT "short $_\[xsize][ysize];\n";}
foreach (@list16u) {print OUT "unsigned short $_\[xsize][ysize];\n";}
foreach (@list32r) {print OUT "float $_\[xsize][ysize];\n";}
...
//for(y=$tilebuffer; y<ysize-$tilebuffer; y++) {for(x=$tilebuffer; x<xsize-$tilebuffer; x++) {pf[x][y]=(int) TEMPIN.get();}} TEMPIN.close();
...
"; 
foreach (@PF){
print OUT"if (pf[x][y]==$_) {training[x][y]=0;}";
}
print OUT"
}}
";
...
for(y=$tilebuffer; y<ysize-$tilebuffer; y++) {for(x=$tilebuffer; x<xsize-$tilebuffer; x++) { if (mask_t[x][y]<$maskclass)  {training[x][y]=0;}}}
...
//TEMPIN.open(filename.c_str(), ios::binary); if(!TEMPIN) {cout << \"error reading $_\" <<endl; exit (1);}
//for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++) {$_\[x][y]=(int) TEMPIN.get();}} TEMPIN.close();

INGDAL = (GDALDataset *) GDALOpen( filename.c_str(), GA_ReadOnly ); INBAND = INGDAL->GetRasterBand(1);
INBAND->RasterIO(GF_Read, 0, 0, xsize, ysize, $_, xsize, ysize, GDT_Byte, 0, 0); GDALClose(INGDAL);
...
filename=\"$cmetrics\\\\\"+tilename+\"\\\\$_\.tif\"; 
//TEMPIN.open(filename.c_str(), ios::binary); if(!TEMPIN) {cout << \"error reading $_\" <<endl; exit (1);}
//for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++) {TEMPIN.read(reinterpret_cast < char * > (&$_\[x][y]), sizeof($_\[x][y]));}} TEMPIN.close();

INGDAL = (GDALDataset *) GDALOpen( filename.c_str(), GA_ReadOnly ); INBAND = INGDAL->GetRasterBand(1);
INBAND->RasterIO(GF_Read, 0, 0, xsize, ysize, $_, xsize, ysize, GDT_Int16, 0, 0); GDALClose(INGDAL);
";}
...
INGDAL = (GDALDataset *) GDALOpen( filename.c_str(), GA_ReadOnly ); INBAND = INGDAL->GetRasterBand(1);
INBAND->RasterIO(GF_Read, 0, 0, xsize, ysize, $_, xsize, ysize, GDT_UInt16, 0, 0); GDALClose(INGDAL);
";}
...
INGDAL = (GDALDataset *) GDALOpen( filename.c_str(), GA_ReadOnly ); INBAND = INGDAL->GetRasterBand(1);
INBAND->RasterIO(GF_Read, 0, 0, xsize, ysize, $_, xsize, ysize, GDT_Float32, 0, 0); GDALClose(INGDAL);
";}
...
print OUT"for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++) {
...
print OUT"Y[k]<<(int)training[x][y]";
$inc=0; foreach (@mainlist){print OUT "<<\",\"<<(int)$_\[x][y]";}
$inc=0; foreach (@diff){($out,$in1,$in2)=split('\,',$_); print OUT "<<\",\"<<(int)$out\[x][y]";#<<\",\"<<avediff[$inc]<<\",\"<<avediff5[$inc]
 ++$inc;}
foreach (@list16s) {print OUT "<<\",\"<<$_\[x][y]";}
$inc=0; foreach (@list16u) {print OUT "<<\",\"<<$_\[x][y]";#<<\",\"<<avemain[$inc]<<\",\"<<avemain5[$inc]
 ++$inc; }
foreach (@list32r) {print OUT "<<\",\"<<$_\[x][y]";}
print OUT"
<<endl;
}}}}}
";
...
//int avediff5[$numdiff];
uint8_t mask_t[xsize][ysize];
uint8_t output[xsize][ysize]; memset(output, 0, sizeof(output[0][0]) * xsize * ysize);
unsigned short pf[xsize][ysize];
";
foreach (@mainlist){print OUT "uint8_t $_\[xsize][ysize];\n";}
foreach (@diff){($out,$in1,$in2)=split('\,',$_); print OUT "unsigned short $out\[xsize][ysize];\n";}
...
filename=\"$cmetrics\\\\\"+tilename+\"\\\\pf.tif\";
//TEMPIN.open(filename.c_str(), ios::binary); if(!TEMPIN) {cout << \"error reading $tile\" <<endl; exit (1);}
//for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++) {pf[x][y]=(int) TEMPIN.get();}} TEMPIN.close();
...
TEMPIN.open(filename.c_str(), ios::binary); if(!TEMPIN) {cout << \"error reading $tile\" <<endl; exit (1);}
for(y=$tilebuffer; y<ysize-$tilebuffer; y++) {for(x=$tilebuffer; x<xsize-$tilebuffer; x++) {mask_t[y][x]=(int) TEMPIN.get();}} TEMPIN.close();
for(y=$tilebuffer; y<ysize-$tilebuffer; y++) {for(x=$tilebuffer; x<xsize-$tilebuffer; x++) {if (mask_t[x][y]>$maskclass) {mask_t[x][y]=1;} else {mask_t[x][y]=0;}}}
";
}
...
$maskclass=1;
print OUT"
for(y=$tilebuffer; y<ysize-$tilebuffer; y++) {for(x=$tilebuffer; x<xsize-$tilebuffer; x++) {mask_t[x][y]=1;}}
";
}
...
for(y=$tilebuffer; y<ysize-$tilebuffer; y++) {for(x=$tilebuffer; x<xsize-$tilebuffer; x++) {";
foreach (@PF){
print OUT"if (pf[x][y]==$_) {mask_t[x][y]=0;}";
}
print OUT"
}}
";
...
=cut
$metnumber=2;
$inc=0; foreach (@mainlist){print OUT "array[$metnumber] = $_\[x][y]\;\n"; ++$metnumber; }
$inc=0; foreach (@diff){($out,$in1,$in2)=split('\,',$_); print OUT "array[$metnumber] = $out\[x][y]\;\n"; ++$metnumber; 
#print OUT "array[$metnumber] = avediff[$inc]\;\n"; ++$inc; ++$metnumber;
# print OUT "array[$metnumber] = avediff5[$inc]\;\n"; ++$metnumber;
}
foreach (@list16s) {print OUT "array[$metnumber] = $_\[x][y]\;\n"; ++$metnumber;}

$inc=0; foreach (@list16u) {print OUT "array[$metnumber] = $_\[x][y]\;\n"; ++$metnumber; 
#print OUT "array[$metnumber] = avemain[$inc]\;\n"; ++$inc; ++$metnumber;
# print OUT "array[$metnumber] = avemain5[$inc]\;\n"; ++$metnumber;
}
foreach (@list32r) {print OUT "array[$metnumber] = $_\[x][y]\;\n"; ++$metnumber;}
...
output[x][y]=result[medposition];
}
else {output[x][y]=0;}
}}
filename=tilename+\"\.out\";
outfile.open(filename.c_str(), ios::binary); if(!outfile) {cout << \"error making output file\" <<endl; exit (1);} 
for(y=$tilebuffer; y<ysize-$tilebuffer; y++) {for(x=$tilebuffer; x<xsize-$tilebuffer; x++) {outfile.put(output[y][x]);}}
outfile.close();
return 0;
}
";
